...
function buildWorkspace() {
    ...
    writeFileSync(feedScript, `const rows = ${JSON.stringify(FIXTURES)};\nfor (const row of rows) console.log(JSON.stringify(row));\n`);
...
function fakeScorer(scored) {
    ...
    return {
        ...
        async scoreAll(postings, _resume, _profile, _preferences, onResult) {
...
function silentNotifier(pushed) {
    return {
        enabled: false,
        async notifyInstant(match) { pushed.push(match); },
        async notifyDigest() { },
...
test('a crawl ingests, dedupes, filters, scores and routes', async () => {
    ...
    try {
        repo.saveProfile({ ...emptyProfile(), fullName: 'Ada Lovelace', email: 'ada@example.com' }, null, 'Six years leading design systems across web and mobile. Figma, React, tokens.');
        repo.savePreferences(PREFERENCES);
        const scored = [];
        const pushed = [];
        const crawler = new Crawler(config, repo, fakeScorer(scored), silentNotifier(pushed), createLogger('fatal'));
...
test('a second crawl re-reads the sources but creates no duplicate matches', async () => {
    ...
    try {
        ...
        repo.savePreferences(PREFERENCES);
        const scored = [];
        const crawler = new Crawler(config, repo, fakeScorer(scored), silentNotifier([]), createLogger('fatal'));
        await crawler.run();
...
test('a crawl with no resume does nothing rather than scoring against nothing', async () => {
    ...
    try {
        repo.savePreferences(PREFERENCES);
        const crawler = new Crawler(config, repo, fakeScorer([]), silentNotifier([]), createLogger('fatal'));
...
test('a match below the threshold is stored but not pushed', async () => {
    ...
    try {
        ...
        const pushed = [];
        const crawler = new Crawler(config, repo, fakeScorer([]), silentNotifier(pushed), createLogger('fatal'));
        await crawler.run();
