...
export class ResumeParser {
    ...
    async parse(buffer, fileName, mimeType, existing) {
        ...
        if (text.length < 100) {
            ...
            throw new ResumeParseError('Almost no text could be read from that file. If it is a scan, export a text-based PDF and try again.', 'too_little_text');
        }
        const warnings = [];
        ...
        if (!profile.email)
            warnings.push('No email address was found. Add one in Preferences before applying.');
        if (!profile.phone)
            warnings.push('No phone number was found.');
        if (profile.skills.length === 0)
            warnings.push('No skills could be identified.');
...
export function extractContactDetails(text) {
    ...
    const portfolio = urls.find((url) => {
        ...
        if (emailDomain && lowered.startsWith(emailDomain))
            return false;
