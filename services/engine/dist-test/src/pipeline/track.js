...
export class ReplyTracker {
    ...
    log;
    accessToken = null;
    constructor(config, repo, llm, prompts, log) {
        this.config = config;
        this.repo = repo;
        this.llm = llm;
    ...
    async sweep() {
        ...
        for (const message of messages) {
            ...
            try {
                ...
                if (!canTransition(match.status, target))
                    continue;
    ...
    async fetchMessages(since) {
        ...
        const list = await this.gmail(`/users/me/messages?q=${encodeURIComponent(query)}&maxResults=50`, accessToken);
        const messages = [];
        for (const { id } of list.messages ?? []) {
            const detail = await this.gmail(`/users/me/messages/${id}?format=full`, accessToken);
            messages.push(toMessage(detail));
        }
        return messages;
    }
    async gmail(path, accessToken) {
    ...
    async getAccessToken() {
        ...
        if (!existsSync(path))
            throw new Error(`Gmail credentials not found at ${path}`);
...
export function mentionsCompany(message, company) {
