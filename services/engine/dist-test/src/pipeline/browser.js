...
export class FormBrowser {
     ...
     * what was shown.
     */
    async submit(applyUrl, fields, resumePath, screenshotPath) {
        return this.withPage(applyUrl, async (page) => {
            await this.assertAutomatable(page);
            for (const field of fields) {
                if (!field.selector || field.skipped || !field.value)
                    continue;
                try {
            ...
            if (!looksConfirmed(confirmationText)) {
                throw new ManualInterventionRequired('The form did not show a confirmation after submitting. Check the posting directly.');
            }
            return { confirmationImage, confirmationText };
        });
    }
    // ── internals ────────────────────────────────────────────────────────────
    async withPage(url, work) {
    ...
    async fillOne(page, field, resumePath) {
...
function collectFields() {
    ...
    controls.forEach((element, index) => {
        ...
        const options = tag === 'select'
            ? Array.from(element.options).map((option) => option.label || option.value).filter(Boolean)
            : [];
        out.push({
            selector,
...
export function looksConfirmed(text) {
    ...
    if (positives.some((phrase) => lowered.includes(phrase)))
        return true;
