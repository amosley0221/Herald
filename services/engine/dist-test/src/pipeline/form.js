...
const BUILTIN_RULES = FieldRulesSchema.parse({
    rules: [
...
export function loadFieldRules(path) {
    if (!existsSync(path))
    ...
    try {
        ...
        throw new Error(`Could not read field rules at ${path}: ${cause.message}`);
    }
}
/**
 ...
 * behind their back.
 */
export function mapFields(discovered, rules, context) {
...
function resolveValue(rule, context) {
