...
import { ApplyError, startOfLocalDay } from './pipeline/apply.js';
import { ResumeParseError } from './resume/parse.js';
...
export async function createServer(deps) {
    ...
    app.get('/profile', async (_request, reply) => {
        ...
        if (!stored)
            return reply.code(404).send({ code: 'no_profile', message: 'No resume has been uploaded yet.' });
        return stored.profile;
    });
    app.put('/profile', async (request) => {
        const patch = ProfilePatchSchema.parse(request.body);
        const stored = repo.getProfile();
        const base = stored?.profile ?? emptyProfile();
    ...
    app.post('/resume', async (request, reply) => {
        ...
        if (!file)
            return reply.code(400).send({ code: 'no_file', message: 'No resume file was uploaded.' });
 ...
 */
export function defaultPreferences(config) {
    return {
        roles: [],
        locations: [],
        remote: true,
