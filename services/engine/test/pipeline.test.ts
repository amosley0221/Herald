...
test('different jobs do not share a dedupe key', () => {
  assert.notEqual(
    dedupeKey(posting({ title: 'Product Designer' })),
    dedupeKey(posting({ title: 'Product Manager' })),
  );
  assert.notEqual(
    dedupeKey(posting({ location: 'Charlotte, NC' })),
    dedupeKey(posting({ location: 'Austin, TX' })),
  );
});
...
test('prefilter drops on role, keyword, salary and staleness', () => {
  const cases: Array<[string, RawPosting]> = [
...
test('form fields map onto profile values', () => {
  ...
  const mapped = mapFields(
    ...
    rules,
    { profile: PROFILE, coverLetter: 'Dear team', resumeFileName: 'ada.pdf' },
  );
...
test('unrecognised controls are surfaced blank rather than guessed at', () => {
  ...
  const mapped = mapFields(
    [{ selector: '#q1', label: 'What is your favourite typeface?', name: 'q1', id: 'q1', kind: 'text', required: false, options: [] }],
    rules,
    { profile: PROFILE, coverLetter: null, resumeFileName: null },
  );
