...
import { FcmSender, type FcmMessage } from './fcm.js';
...
export class Notifier {
  ...
  async notifyInstant(match: Match, preferences: Preferences): Promise<void> {
  ...
  async notifyDigest(matches: Match[], preferences: Preferences): Promise<void> {
    if (!preferences.digest || matches.length === 0) return;
    await this.broadcast({
    ...
    for (const match of matches) this.repo.markNotified(match.id, 'digest');
  }

  /** A submission that Herald must not complete on the user's behalf. */
  async notifyNeedsYou(match: Match, reason: string): Promise<void> {
    await this.broadcast({
  ...
  private async broadcast(message: Omit<FcmMessage, 'token'>): Promise<void> {
