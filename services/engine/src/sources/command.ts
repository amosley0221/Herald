...
import {
  htmlToText, looksRemote, parsePayText, readPath, stringList,
 ...
 */
export const commandAdapter: SourceAdapter = {
  id: 'command',
  ...
  async *fetch(source: SourceConfig, ctx: FetchContext): AsyncIterable<RawPosting> {
    ...
    const args = stringList(source, 'args').map((arg) =>
      arg
    ...
    const records = await runCommand(command, args, {
      ...
      env: (source.options.env as Record<string, string> | undefined) ?? {},
      signal: ctx.signal,
      format,
      listPath: source.options.listPath as string | undefined,
      log: ctx.log,
    });
...
async function runCommand(
  ...
  args: string[],
  opts: {
    timeoutMs: number; cwd?: string; env: Record<string, string>; signal: AbortSignal;
    format: string; listPath?: string; log: FetchContext['log'];
  },
): Promise<unknown[]> {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (opts.format === 'ndjson') {
        ...
        if (tail) {
          try { records.push(JSON.parse(tail)); } catch { /* partial final line */ }
        }
        resolve(records);
        return;
      }
...
function toRawPosting(record: unknown, source: SourceConfig): RawPosting | null {
