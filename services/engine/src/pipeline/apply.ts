...
import type { Match, PreparedApplication, Preferences, Profile } from '@herald/core';
...
import { fallbackFields, loadFieldRules, mapFields, type FieldRule, type MappedField } from './form.js';
...
export class ApplyService {
  ...
  private readonly browser: FormBrowser;
   ...
   */
  async approve(matchId: string): Promise<PreparedApplication> {
    const match = this.repo.getMatch(matchId);
    if (!match) throw new ApplyError(`No match with id ${matchId}`, 'not_found');

    const existing = this.repo.getPrepared(matchId);
    if (existing && match.status !== 'pending') {
      return { matchId, ...existing, fields: existing.fields as PreparedApplication['fields'] };
    }
    ...
    if (!stored) throw new ApplyError('No resume has been uploaded yet', 'no_profile');
  ...
  async submit(
    matchId: string,
    overrides?: { fields?: Record<string, string>; coverLetter?: string },
  ): Promise<Match> {
    const match = this.repo.getMatch(matchId);
    if (!match) throw new ApplyError(`No match with id ${matchId}`, 'not_found');
    ...
    if (!prepared) throw new ApplyError('Approve the match before submitting it', 'not_prepared');
    ...
    const fields = (prepared.fields as MappedField[]).map((field) => ({
      ...field,
    ...
    try {
      const { confirmationImage } = await this.browser.submit(
        match.posting.applyUrl,
        fields,
        stored?.resumePath ?? null,
        screenshotPath,
      );
  ...
  private async routeToUser(match: Match, reason: string, payload: Record<string, string>): Promise<Match> {
    const at = new Date().toISOString();
   ...
   */
  private manualOnlyReason(match: Match): string | null {
    let host: string;
    try {
    ...
    const blocked = this.config.apply.manualOnlyHosts.some(
  ...
  private async writeCoverLetter(match: Match, profile: Profile, log: Logger): Promise<string | null> {
...
function stripSelectors(fields: MappedField[]): PreparedApplication['fields'] {
  return fields.map(({ selector, skipped, ...field }) => field);
}

/** Midnight in the user's zone, as an ISO instant, for the daily cap window. */
export function startOfLocalDay(timeZone: string | undefined, now: Date = new Date()): string {
  if (!timeZone) {
