...
const TITLE_SYNONYMS: ReadonlyArray<readonly [RegExp, string]> = [
  [/\bsr\.?\b/gi, 'senior'],
  [/\bjr\.?\b/gi, 'junior'],
  [/\bmgr\.?\b/gi, 'manager'],
  [/\beng\.?\b/gi, 'engineer'],
...
export function normalizeTitle(title: string): string {
  ...
  for (const [pattern, replacement] of TITLE_SYNONYMS) out = out.replace(pattern, replacement);
 ...
 */
export function normalizeLocation(location: string, remote: boolean): string {
  if (remote) return 'remote';
  const trimmed = location.toLowerCase().trim();
  if (!trimmed) return '';
  if (/\b(remote|anywhere)\b/.test(trimmed)) return 'remote';
 ...
 */
export function dedupeKey(posting: Pick<RawPosting, 'company' | 'title' | 'location' | 'remote'>): string {
  const parts = [
...
export function contentHash(
  posting: Pick<RawPosting, 'company' | 'title' | 'location' | 'description'>,
 ...
 */
export function collapse<T extends { dedupeKey: string; sourcePriority: number; postedAt: string }>(
  postings: T[],
): T[] {
  const best = new Map<string, T>();
  for (const posting of postings) {
    const incumbent = best.get(posting.dedupeKey);
    if (!incumbent || wins(posting, incumbent)) best.set(posting.dedupeKey, posting);
  }
  return [...best.values()];
}

function wins(
  candidate: { sourcePriority: number; postedAt: string },
  incumbent: { sourcePriority: number; postedAt: string },
): boolean {
