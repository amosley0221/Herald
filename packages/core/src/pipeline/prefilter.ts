...
export interface PrefilterResult {
  keep: boolean;
  /** Why a posting was dropped, for the crawl log. Null when kept. */
  reason: string | null;
}
...
export function prefilter(
  ...
  options: { maxPostingAgeDays: number; now?: Date } = { maxPostingAgeDays: 30 },
): PrefilterResult {
  const now = options.now ?? new Date();

  const postedAt = new Date(posting.postedAt);
  if (!Number.isFinite(postedAt.getTime())) return drop('unparseable posting date');
  const ageDays = (now.getTime() - postedAt.getTime()) / 86_400_000;
  if (ageDays > options.maxPostingAgeDays) return drop(`older than ${options.maxPostingAgeDays} days`);
  ...
  for (const exclude of preferences.excludeKeywords) {
    if (!exclude.trim()) continue;
    if (haystack.includes(exclude.toLowerCase().trim())) return drop(`excluded by keyword "${exclude}"`);
  }
...
function drop(reason: string): PrefilterResult {
 ...
 */
export function matchesAnyRole(title: string, roles: string[]): boolean {
...
function matchesAnySeniority(title: string, seniority: string[]): boolean {
  return seniority.some((level) => title.includes(level.toLowerCase().trim()));
}

function matchesLocation(posting: RawPosting, preferences: Preferences): boolean {
  if (posting.remote && preferences.remote) return true;
  ...
  if (!location) return true; // unknown location; let scoring judge it
